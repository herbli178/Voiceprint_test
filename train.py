...
add_arg('num_epoch',        int,    22,                       '训练的轮数')
add_arg('num_classes',      int,    8,                     '分类的类别数量')
add_arg('learning_rate',    float,  1e-2,                     '初始学习率的大小')
add_arg('input_shape',      str,    '(257, 257, 1)',          '数据输入的形状')
add_arg('train_list_path',  str,    'train_list.txt', '训练数据的数据列表路径')
add_arg('test_list_path',   str,    'test_list.txt',  '测试数据的数据列表路径')
